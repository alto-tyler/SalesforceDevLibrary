...
const ClearSelectionButton = "Clear Selection";
const ClearFilterButton = "Clear All Filters";
const SetFilterAction = "Set Filter";
const ClearFilterAction = "Clear Filter";
const ColumnHeader = "Column";
const FilterHeader = "Filter";
const LabelHeader = "Label";
const RequiredMessage = "This is a required entry. At least 1 row must be selected.";
const EmptyMessage = "This table currently has no data to display";
...
const ShowingPagePrefix = "Showing Page";
const ShowingPageMiddle = "of";
const ShowingPageSuffix = "Pages";
const FilterBlankLabel = "Filter Blank Values?";
const FilterBlankHelpText = "Check here if you want to filter rows where the value in this column is blank";

const CONSTANTS = getConstants();   // From alto_datatableUtils
...
export default class Datatable extends LightningElement {
    ...
    @api
    get columnCellAttribs() {
        return this._columnCellAttribs;
    }
    set columnCellAttribs(value) {
        this._columnCellAttribs = value;
        console.log(this.consoleLogPrefix+'columnCellAttribs set to:', (SHOW_DEBUG_INFO) ? JSON.stringify(value) : '***');
        this.connectedCallback();
    }
    _columnCellAttribs = [];
    @api 
    get columnEdits() {
        return this._columnEdits;
    }
    set columnEdits(value) {
        this._columnEdits = value;
        this.connectedCallback();
    }
    _columnEdits = '';
    ...
    _columnLabels = [];
    @api
    get columnOtherAttribs() {
        return this._columnOtherAttribs;
    }
    set columnOtherAttribs(value) {
        this._columnOtherAttribs = value;
        this.connectedCallback();
    }
    _columnOtherAttribs = [];
    ...
    _outputSelectedRows = [];
    @api
    get outputSelectedRows() {
        return this._outputSelectedRows;
    }
    set outputSelectedRows(value) {
        this._outputSelectedRows = value;
    }
    _outputSelectedRow = {};
    @api
    get outputSelectedRow() {
        return this._outputSelectedRow;
    }
    set outputSelectedRow(value) {
        this._outputSelectedRow = value;
    }
    _outputEditedRows = [];
    @api
    get outputEditedRows() {
        return this._outputEditedRows;
    }
    set outputEditedRows(value) {
        this._outputEditedRows = value;
    }
    _outputAllRows = [];
    @api
    get outputAllRows() {
        // Return a merged view: edited rows take priority over original table data.
        // Edited rows that do not exist in the original table are appended.
        try {
            const original = Array.isArray(this._tableData) ? JSON.parse(JSON.stringify(this._tableData)) : [];
            const edited = Array.isArray(this._editedData) ? JSON.parse(JSON.stringify(this._editedData)) : [];

            if ((!edited || edited.length === 0) && original.length === 0) {
                return [];
            }
            ...
            edited.forEach(r => {
                if (r && typeof r === 'object' && r[keyField] != null) {
                    editedMap.set(String(r[keyField]), r);
                }
            });

            const merged = [];

            // Walk originals and replace with edited where present
            original.forEach(orig => {
                const id = orig && orig[keyField] != null ? String(orig[keyField]) : null;
                if (id && editedMap.has(id)) {
                    merged.push(editedMap.get(id));
                    editedMap.delete(id);
                } else {
                    merged.push(orig);
                }
            ...
            // Append any edited rows that didn't match an original (new rows)
            if (editedMap.size > 0) {
                editedMap.forEach(v => merged.push(v));
            }
            console.log(this.consoleLogPrefix+'outputAllRows - ',(SHOW_DEBUG_INFO) ? JSON.stringify(merged) : '***');
            this.notifyFlowAttribute('outputAllRows', merged);
            this._numberOfRowsAll = merged.length;
            this.notifyFlowAttribute('numberOfRowsAll', this._numberOfRowsAll);
            return merged;
        } catch (e) {
            console.error(this.consoleLogPrefix+'Error computing outputAllRows:', e);
            const rows = this._outputAllRows || [];
            // Fallback to stored value on error
            this.notifyFlowAttribute('outputAllRows', this._outputAllRows || []);
            this._numberOfRowsAll = rows.length;
            this.notifyFlowAttribute('numberOfRowsAll', this._numberOfRowsAll || 0);
            return this._outputAllRows || [];
        }
    }
    set outputAllRows(value) {
        this._outputAllRows = value;
        
    }
    @api
    get numberOfRowsAll() {
        this._numberOfRowsAll =this.outputAllRows.length;
        return this._numberOfRowsAll || 0;
    }
    set numberOfRowsAll(value) {
        this._numberOfRowsAll = value;
    }
    _numberOfRowsAll;
    ...
    get isRemoveRowAction() {
        return (this._isRemoveRowAction) ? true : false;
    }
    ...
    // Notify Flow helper: dispatch FlowAttributeChangeEvent only when value changed
    notifyFlowAttribute(name, value) {
        try {
            this.dispatchEvent(new FlowAttributeChangeEvent(name, value));
            console.debug(this.consoleLogPrefix+`Dispatching FlowAttributeChangeEvent for ${name}:`, (SHOW_DEBUG_INFO) ? JSON.stringify(value) : '***');
        } catch (e) {
            // Best-effort: still dispatch if something goes wrong in comparison
            this.dispatchEvent(new FlowAttributeChangeEvent(name, value));
        }
    }
        ...
        if(this.isUpdateTable) {       
            if (Array.isArray(data)) {
                ...
                if(this.columnFields) {
                    ...
                    // Notify Flow that the merged/all rows output may have changed
    ...
    @api 
    get isRequired() {
        return (this._isRequired) ? true : false;
    }
    ...
    get hideCheckboxColumn() {
        return (this._hideCheckboxColumn) ? true : false;
    }
    ...
    get hideHeaderActions() {
        return (this._hideHeaderActions) ? true : false;
    }
    set hideHeaderActions(value) {
        this._hideHeaderActions = value;
    }
    _hideHeaderActions;

    @api 
    get hideClearSelectionButton() {
        return (this._hideClearSelectionButton) ? true : false;
    }
    set hideClearSelectionButton(value) {
        this._hideClearSelectionButton = value;
    }
    _hideClearSelectionButton;

    @api 
    get showRowNumbers() {
        return (this._showRowNumbers) ? true : false;
    }
    ...
    @api 
    get showPagination() {
        return (this._showPagination) ? true : false;
    }
    ...
    get showFirstLastButtons() {
        return (this._showFirstLastButtons) ? true : false;
    }
    ...
    @api 
    get showRecordCount() {
        return (this._showRecordCount) ? true : false;
    }
    ...
    get showSelectedCount() {
        return (this._showSelectedCount) ? true : false;
    }
    ...
    get singleRowSelection() {
        return (this._singleRowSelection) ? true : false;
    }
    ...
    get suppressBottomBar() {
        return (this._suppressBottomBar) ? true : false;
    }
    ...
    get matchCaseOnFilters() {
        return (this._matchCaseOnFilters) ? true : false;
    }
    
    @api 
    get tableBorder() {
        console.log(this.consoleLogPrefix+'Table Border value: '+this._tableBorder);
        return (this._tableBorder) ? true : false;
    }
    ...
    @api 
    get isDisplayHeader() {
        return (this._isDisplayHeader) ? true : false;
    }
    ...
    @api 
    get isShowSearchBar() {
        return(this._isShowSearchBar) ? true : false;
    }
    ...
    set not_suppressNameFieldLink(value) {
        this._not_suppressNameFieldLink = value;
    }
    _not_suppressNameFieldLink;

    @api
    get openLinkinSameTab() {
        return (this._openLinkinSameTab) ? true : false;
    }
    set openLinkinSameTab(value) {
        this._openLinkinSameTab = value;
    }
    _openLinkinSameTab;
    ...
    @api 
    get displayAll() {
        return (this._displayALl) ? true : false;
    }
    set displayAll(value) {
        this._displayALl = value;
    }
    _displayALl;

    @api 
    get isUserDefinedObject() {
        return (this._isUserDefinedObject) ? true : false;
    }
    ...
    set serializedRecordData(value) {
        if(this.isSerializedRecordData && this.isUpdateTable) {
            ...
            this.dispatchEvent(new FlowAttributeChangeEvent('outputAllRows', this.outputAllRows));
            ...
            this.notifyFlowAttribute('outputEditedSerializedRows', this.outputEditedSerializedRows);
            this.notifyFlowAttribute('outputAllRows', this.outputAllRows);
            setTimeout(function() {
                this.connectedCallback();
            }.bind(this), 1000);
    ...
    get isSerializedRecordData(){
        return (this._isSerializedRecordData) ? true : false;
    } 
    set isSerializedRecordData(value){
        this._isSerializedRecordData = value;
    }
    _isSerializedRecordData;

    @api 
    get allowNoneToBeChosen() {
        return (this._allowNoneToBeChosen) ? true : false;
    }
    set allowNoneToBeChosen(value) {
        this._allowNoneToBeChosen = value;
    }
    _allowNoneToBeChosen;

    @api 
    get allowOverflow() {
        return (this._allowOverflow) ? true : false;
    }
    ...
    get suppressCurrencyConversion() {
        return (this._suppressCurrencyConversion) ? true : false;
    }
    set suppressCurrencyConversion(value) {
        this._suppressCurrencyConversion = value;
    }
    _suppressCurrencyConversion;

    @api 
    get isCaseInsensitiveSort() {
        return (this._isCaseInsensitiveSort) ? true : false;
    }
    ...
    get formElementClass() {
        return this.isInvalid ? 'slds-form-element slds-has-error slds-m-bottom_x-small' : 'slds-form-element slds-m-bottom_x-small';
    }
    ...
    connectedCallback() {
        ...
        // Pagination state (if you want to reset to first page)
        this.pageCurrentNumber = 1;
    ...
    processDatatable() {
        if (this.isUserDefinedObject) {
            ...
            if (!this.isUserDefinedObject) {
                ...
                data.forEach(record => { 
                    delete record['attributes'];    // When running the Column Wizard, clean up the record string before getting the field details from alto_DatatableController
                });
            }
    ...
    updateColumns() {
        ...
        if (this.hasRowAction && this.rowActionLeftOrRight == "Left") this.addRowAction();
        ...
        this.basicColumns.forEach(colDef => {
            ...
            this.cols.push({
                ...
                sortable: (this.hideHeaderActions) ? false : true,
...
// TODO - Add support for User Defined Validation Rules
    ...
    handleHeaderAction(event) {
        ...
        const prompt = this.label.FilterHeader;
    ...
    setWidth(sizes) {
        ...
        this.basicColumns.forEach(colDef => {
            colFlexWidth = this.columns[colNum].actions?.find(a => a.name == 'flex_'+colNum)?.checked ? 0 : sizes[colNum];   // v4.3.1 Reset column width when Flex is toggled off
